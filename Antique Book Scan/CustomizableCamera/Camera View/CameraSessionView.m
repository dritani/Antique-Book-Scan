...
@interface CameraSessionView () <CaptureSessionManagerDelegate>
{
    ...
    CGSize photoLibraryButtonSize;
    CGSize leftSnailButtonSize;
    CGSize rightRabbitButtonSize;
...
@property (readwrite) BOOL beginning;
...
@property (nonatomic, strong) UIButton *leftSnailButton;
@property (nonatomic, strong) UIButton *rightRabbitButton;
...
-(void)setupCaptureManager:(CameraType)camera {
    ...
    if (_beginning) {
        ...
        /*
        UIImageView *imageHolder = [[UIImageView alloc] initWithFrame:self.frame];
        UIImage *image = [UIImage imageNamed:@"leap_camera.jpg"];
...
-(void)composeInterface {
    ...
    UILabel *denied = [UILabel new];
    denied.text = @"Please authorize the camera by going to Settings->Privacy->Camera";
    denied.textColor = [UIColor whiteColor];
    [denied sizeToFit];
    denied.frame = (CGRect){0,0, CGSizeMake(self.bounds.size.width * 0.2, self.bounds.size.height * 0.2)};
    denied.center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
    denied.hidden = YES;
    [self addSubview:denied];
    ...
    */
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
    {
        ...
        photoLibraryButtonSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.04, [[UIScreen mainScreen] bounds].size.height * 0.04);
        leftSnailButtonSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.04, [[UIScreen mainScreen] bounds].size.height * 0.04);
        rightRabbitButtonSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.04, [[UIScreen mainScreen] bounds].size.height * 0.04);
    } else
    {
        ...
        photoLibraryButtonSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.05, [[UIScreen mainScreen] bounds].size.height * 0.05);
        leftSnailButtonSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.05, [[UIScreen mainScreen] bounds].size.height * 0.05);
        rightRabbitButtonSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.05, [[UIScreen mainScreen] bounds].size.height * 0.05);
    }
    

    // create photo library button
    _photoLibraryButton = [UIButton new];

    // create snail and rabbit buttons
    _leftSnailButton = [UIButton new];
    _rightRabbitButton = [UIButton new];
    ...
    if (_captureManager) {
        ...
        //Button Visual attribution
        _photoLibraryButton.frame = (CGRect){0,0, photoLibraryButtonSize};
        _photoLibraryButton.center = CGPointMake(10+photoLibraryButtonSize.width, self.frame.size.height*0.9125);
        _photoLibraryButton.tag = PhotoLibraryButtonTag;
        _photoLibraryButton.backgroundColor = [UIColor clearColor];
        [_photoLibraryButton setBackgroundImage:[UIImage imageNamed:@"photoAlbum.png"] forState:UIControlStateNormal];
                                                ...
                                                resultHandler:^(UIImage *result, NSDictionary *info) {
                                                    ...
                                                    dispatch_async(dispatch_get_main_queue(), ^{
                                                        ...
                                                        if (result == NULL) {
                                                            ...
                                                            [_photoLibraryButton setBackgroundImage:result forState:UIControlStateNormal];
                                                        }
                                                    });
                                                }];
        */
        ...
        //Button target
        [_photoLibraryButton addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_photoLibraryButton];

        // snail button
        //Button Visual attribution
        _leftSnailButton.frame = (CGRect){0,0, leftSnailButtonSize};
        _leftSnailButton.center = CGPointMake(self.frame.size.width-10-photoLibraryButtonSize.width-leftSnailButtonSize.width, self.frame.size.height*0.9125);
        _leftSnailButton.tag = LeftSnailButtonTag;
        [_leftSnailButton setBackgroundImage:[UIImage imageNamed:@"left off.png"] forState:UIControlStateNormal];
        //Button target
        [_leftSnailButton addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_leftSnailButton];

        // rabbit button
        //Button Visual attribution
        _rightRabbitButton.frame = (CGRect){0,0, rightRabbitButtonSize};
        _rightRabbitButton.center = CGPointMake(self.frame.size.width-10-photoLibraryButtonSize.width, self.frame.size.height*0.9125);
        _rightRabbitButton.tag = RightRabbitButtonTag;
       

        //Button target
        [_rightRabbitButton addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_rightRabbitButton];

        
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        NSInteger speed;
        
        if([[[defaults dictionaryRepresentation] allKeys] containsObject:@"speed"]){
            
            speed = [defaults integerForKey:@"speed"];
            if (speed == 1) {
                [_leftSnailButton setBackgroundImage:[UIImage imageNamed:@"left on.png"] forState:UIControlStateNormal];
                [_rightRabbitButton setBackgroundImage:[UIImage imageNamed:@"right off.png"] forState:UIControlStateNormal];
            } else if (speed == 2) {
                [_leftSnailButton setBackgroundImage:[UIImage imageNamed:@"left off.png"] forState:UIControlStateNormal];
                [_rightRabbitButton setBackgroundImage:[UIImage imageNamed:@"right on.png"] forState:UIControlStateNormal];
            }
        } else {
            [_leftSnailButton setBackgroundImage:[UIImage imageNamed:@"left off.png"] forState:UIControlStateNormal];
            [_rightRabbitButton setBackgroundImage:[UIImage imageNamed:@"right on.png"] forState:UIControlStateNormal];
            [defaults setInteger:2 forKey:@"speed"];
    ...
    if (_topBarView) {
        ...
        _topBarView.backgroundColor = [UIColor colorWithRed: 0.098 green: 0.090 blue: 0.098 alpha: 0.64];
...
-(void)inputManager:(id)sender {
    ...
    switch ([(UIButton *)sender tag]) {
        ...
        case PhotoLibraryButtonTag: [self onTapPhotoButton];  return;
        case LeftSnailButtonTag: [self onTapSnailButton];  return;
        case RightRabbitButtonTag: [self onTapRabbitButton];  return;
    }
}
...
- (void)onTapPhotoButton {
    [self.delegate didTapPhotoLibrary];

}

- (void)onTapSnailButton {
    [_leftSnailButton setBackgroundImage:[UIImage imageNamed:@"left on.png"] forState:UIControlStateNormal];
    [_rightRabbitButton setBackgroundImage:[UIImage imageNamed:@"right off.png"] forState:UIControlStateNormal];
...
//}
//
- (void)hideSnailRabbitButtons
{
    _leftSnailButton.hidden = YES;
    _rightRabbitButton.hidden = YES;
}
